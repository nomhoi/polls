...
В список responses можно включать только те ответы, которые были изменены в клиентском приложении.
Id ответа на вопрос нужно включать только для тех ответов, для которых был указан respond_id в теле ответа на запроса GET /api/v1/polls/:id/ (см. UserResponseAPITestCase::test_update).
